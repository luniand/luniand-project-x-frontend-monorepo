...
export const useTheme = (
  ...
  const rawClasses = computed(() => {
    if (unref(userTheme)?.[namespace])
      return R.mergeRight(
        defaultTheme.classes,
        unref(userTheme)[namespace].classes || {}
      );

    return defaultTheme.classes;
  });

  const classPrefix = computed(() => unref(userTheme)?.classPrefix ?? "lu-");
